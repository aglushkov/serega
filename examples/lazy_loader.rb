...
class UserSerializer < AppSerializer
  lazy_loader :users_posts, UsersPostsLoader

  attribute :first_name
  attribute :last_name
  attribute :posts, serializer: "PostSerializer", many: true, lazy: :users_posts, value: proc { |obj, lazy:|
    lazy[:users_posts][obj.id]
  }
end

class PostSerializer < AppSerializer
  lazy :posts_comments, PostsCommentsLoader

  attribute :text
  attribute :comments, serializer: "CommentSerializer", many: true, lazy: :posts_comments, value: proc { |obj, lazy:|
    lazy[:posts_comments][obj.id]
  }
end

class CommentSerializer < AppSerializer
  lazy :comments_views, CommentsViewsLoader

  attribute :text
  attribute :views_count, delegate: {to: :view, method: :count}, value: proc { |obj, lazy:|
    lazy[:comments_views][obj.id]
  }
end
